/**
 * Cluster analysis pipeline — outputs to MongoDB instead of console
 *
 * Source: normalized_posts collection
 * Output: Same database, writes to:
 *   - account_enriched_clusters  – one document per clustered post, fully enriched
 *   - daily_emotion              – one document per calendar day
 *   - analysis_runs              – summary of each run
 *
 * Usage:
 *   npx tsx src/scripts/run-cluster-analysis-to-db.ts [options]
 *
 * Options:
 *   --mongo-uri <uri>           MongoDB connection string (default: mongodb://only-facts:ybqX07w0iPccW5TJ3eWl80OO@localhost:27017/only-facts)
 *   --window-hours <n>          Time window for clustering (default: 24)
 *   --similarity-threshold <n>  Fixed threshold (auto-detect if omitted)
 *   --min-cluster-size <n>      Minimum posts per cluster (default: 3)
 *   --min-token-length <n>      Minimum token length for TF-IDF (default: 3)
 *   --top-terms <n>             Top terms per cluster (default: 8)
...
const analysisRunSchema = new Schema(
  ...
  { collection: 'analysis_runs', versionKey: false, timestamps: true }
);
...
// ─────────────────────────────────────────────────────────────────────────────
// Optimal threshold search (Calinski-Harabasz)
// ─────────────────────────────────────────────────────────────────────────────
...
function greedyCluster(
  ...
  for (const vec of vectors) {
    ...
    if (bestK >= 0) {
      ...
      sAddWeighted(sums[bestK], vec, 1);
      centroids[bestK] = new Map(sums[bestK]);
      sNormalize(centroids[bestK]);
      sizes[bestK] += 1;
    } else {
      assignments.push(centroids.length);
      sums.push(new Map(vec));
...
function parseArgs(argv: string[]): CliOptions {
  ...
  return {
    mongoUri:
      args.get('--mongo-uri') ||
      'mongodb://only-facts:ybqX07w0iPccW5TJ3eWl80OO@localhost:27017/only-facts',
...
async function main(): Promise<void> {
  ...
  const runId = `analysis-${new Date().toISOString()}`;

  console.log(`Connecting to: ${opts.mongoUri}`);
  const conn: Connection = mongoose.createConnection(opts.mongoUri);
  await new Promise<void>((resolve, reject) => {
    conn.once('open', resolve);
    conn.once('error', reject);
  });

  const AccountEnrichedModel = conn.model('AccountEnriched', accountEnrichedSchema);
  const DailyEmotionModel = conn.model('DailyEmotion', dailyEmotionSchema);
  ...
  try {
    ...
    let cursor = normalizedPostsCollection.find(dbQuery).sort({ date: 1 }).project({
      _id: 1,
      date: 1,
      account: 1,
      content: 1,
    ...
    const rawPosts: NormalizedPost[] = (await cursor.toArray()) as NormalizedPost[];

    if (rawPosts.length === 0) {
      console.log('No posts found.');
      await AnalysisRunModel.updateOne(
        { runId },
        { status: 'completed', completedAt: new Date(), 'results.postsProcessed': 0 }
      );
      return;
    }
    ...
    const clusteringInput: BaselineClusteringPost[] = rawPosts.map((p) => ({
      ...
      referencePostId: p.referencePostId ?? null,
      isRetweet: p.metadata?.isRetweet ?? null,
    ...
    // Update config with computed threshold
    await AnalysisRunModel.updateOne(
      { runId },
    ...
    // ── 5. Account velocity + NLP signals ────────────────────────────────────
    ...
    // ── 6. Daily emotion + per-author avg ────────────────────────────────────
    ...
    // Persist daily_emotion (for this run)
    await DailyEmotionModel.deleteMany({ runId });
    ...
    // ── 7. Build and persist account_enriched_clusters ───────────────────────
    ...
    // ── Summary — write to analysis_runs collection ──────────────────────────
    console.log('\n');
    ...
    const topDangerDays = dailyRecords
      ...
      .map((d) => ({
        ...
        dominantEmotionDangerLevel: d.dominantEmotionDangerLevel,
    ...
    console.log(`✓ Analysis complete. Results saved to analysis_runs collection.`);
    console.log(`  Run ID: ${runId}`);
    console.log(`  Posts processed: ${results.postsProcessed}`);
    console.log(`  Posts clustered: ${results.postsClustered}`);
    console.log(`  Clusters found: ${results.clusters}`);
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : String(err);
    await AnalysisRunModel.updateOne(
      { runId },
      { status: 'failed', completedAt: new Date(), error: errorMessage }
    );
    throw err;
  } finally {
    await conn.close();
